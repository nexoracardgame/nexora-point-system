...
const th = {
  ...
  "layout.page.community": "คอมมูนิตี้",
  "layout.page.redeem": "แลกของ",
  ...
  "layout.nav.community": "คอมมูนิตี้",
  "layout.nav.chat": "แชท",
  ...
  "notifications.new": (vars: TranslationVars) => `${vars.count} ใหม่`,
  "notifications.loading": "กำลังโหลดการแจ้งเตือน...",
  "notifications.empty": "ยังไม่มีการแจ้งเตือน",
  "notifications.justNow": "เมื่อสักครู่",
  "notifications.minutes": (vars: TranslationVars) => `${vars.count} นาที`,
  "notifications.hours": (vars: TranslationVars) => `${vars.count} ชม.`,
  "notifications.days": (vars: TranslationVars) => `${vars.count} วัน`,
  ...
  "deals.autoRefresh":
    "ดีลจะรีเฟรชอัตโนมัติเมื่อกลับมาที่หน้านี้และทุก 15 วินาที",
  ...
  "deals.role.ownerAction": "เจ้าของต้องตัดสินใจ",
  ...
  "deals.acceptedDesc":
    "นัดเจอ ตรวจสภาพการ์ดจริง แล้วให้ผู้ซื้อกรอก serial เพื่อปิดการขายจริง",
  ...
  "market.card.completedDeals": (vars: TranslationVars) =>
    `อิงจากดีลที่ปิดจริง ${vars.count} รายการของการ์ด No.${vars.cardNo}`,
  "market.card.askingPrices": (vars: TranslationVars) =>
    `อิงจากราคาตั้งขาย ${vars.count} รายการของการ์ด No.${vars.cardNo}`,
  "market.card.medianHint":
    "ตัวเลขกลางใช้ median เพื่อกันโพสต์ราคาเวอร์ลากค่าขึ้น",
  ...
  "market.card.noOffers": "ยังไม่มีความเคลื่อนไหวของข้อเสนอ",
  "market.card.rarityShare": "สัดส่วนความหายากในตลาด",
  "market.card.rarityDesc": (vars: TranslationVars) =>
    `${vars.rarity} มีอยู่ ${vars.same} จาก ${vars.total} รายการที่เปิดขายอยู่ในตลาด`,
  "market.card.rarityHint":
    "ค่านี้ใช้จากรายการที่เปิดขายจริงในตลาด ไม่ใช่ drop rate ปลอม",
  "market.card.listedToday": "ลงขายวันนี้",
  "market.card.listedOneDay": "ลงขายมาแล้ว 1 วัน",
  "market.card.listedDays": (vars: TranslationVars) =>
    `ลงขายมาแล้ว ${vars.count} วัน`,
  "market.card.unknownUser": "ผู้ใช้ไม่ทราบชื่อ",
  "market.card.completedSale": (vars: TranslationVars) =>
    `${vars.seller} ปิดการขายแล้ว${vars.price ? ` ที่ ${vars.price}` : ""}`,
  "market.card.acceptedOffer": (vars: TranslationVars) =>
    `${vars.seller} ตกลงรับข้อเสนอ${vars.price ? ` ที่ ${vars.price}` : ""}`,
  "market.card.updatedListing": (vars: TranslationVars) =>
    `${vars.seller} อัปเดตรายการนี้`,
...
const en = {
  ...
  "notifications.new": (vars: TranslationVars) => `${vars.count} new`,
  "notifications.loading": "Loading notifications...",
  "notifications.empty": "No notifications yet",
  "notifications.justNow": "Just now",
  "notifications.minutes": (vars: TranslationVars) => `${vars.count}m`,
  "notifications.hours": (vars: TranslationVars) => `${vars.count}h`,
  "notifications.days": (vars: TranslationVars) => `${vars.count}d`,
  ...
  "deals.autoRefresh":
    "Deals auto-refresh when you return to this page and every 15 seconds.",
  ...
  "market.card.completedDeals": (vars: TranslationVars) =>
    `Based on ${vars.count} completed deals for card No.${vars.cardNo}`,
  "market.card.askingPrices": (vars: TranslationVars) =>
    `Based on ${vars.count} asking-price records for card No.${vars.cardNo}`,
  "market.card.medianHint":
  ...
  "market.card.rarityDesc": (vars: TranslationVars) =>
    `${vars.rarity} appears in ${vars.same} of ${vars.total} active market listings`,
  "market.card.rarityHint":
  ...
  "market.card.listedDays": (vars: TranslationVars) =>
    `Listed ${vars.count} days ago`,
  "market.card.unknownUser": "Unknown User",
  "market.card.completedSale": (vars: TranslationVars) =>
    `${vars.seller} completed a sale${vars.price ? ` at ${vars.price}` : ""}`,
  "market.card.acceptedOffer": (vars: TranslationVars) =>
    `${vars.seller} accepted an offer${vars.price ? ` at ${vars.price}` : ""}`,
  "market.card.updatedListing": (vars: TranslationVars) =>
    `${vars.seller} updated this listing`,
} as const satisfies Record<string, TranslationValue>;

export const translations = { th, en } satisfies Record<
  Locale,
...
export function translate(locale: Locale, key: string, vars?: TranslationVars) {
  const table = (translations[locale] ?? translations[DEFAULT_LOCALE]) as Record<
    string,
    TranslationValue
  >;
  const fallbackTable = translations[DEFAULT_LOCALE] as Record<
    string,
    TranslationValue
  >;
  const value = table[key] ?? fallbackTable[key] ?? key;
