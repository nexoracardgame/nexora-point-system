...
export default function MainLayout({
  ...
  const isBattleArenaPage = pathname.startsWith("/battle/triad-dominion");
  const hideFloatingChatOnMobile = pathname === "/dm" || isChatRoomPage || isBattleRoute;
  ...
  return (
    ...
    <div className="min-h-[var(--app-shell-height)] bg-[#050608] text-white">
      ...
      <div className="relative flex min-h-[var(--app-shell-height)]">
        {/* DESKTOP LEFT DOCK */}
        {!isBattleRoute ? <aside className="fixed left-0 top-0 z-[600] hidden h-[var(--app-shell-height)] w-[92px] min-w-[92px] border-r border-white/5 bg-[#0a0b0e]/95 xl:flex xl:flex-col xl:items-center">
        ...
        {/* MAIN */}
        <div className={`relative z-0 flex min-w-0 flex-1 flex-col ${isBattleRoute ? "" : "xl:ml-[92px]"}`}>
          {/* TOPBAR */}
          <header
            className={`top-0 z-[500] border-b border-white/5 bg-[#0b0c10]/88 backdrop-blur-2xl ${
              isBattleRoute ? "hidden" : isChatRoomPage ? "hidden xl:sticky xl:block" : "sticky"
            }`}
          >
          ...
          >
            <div
              className={`${isBattleArenaPage ? "nexora-battle-shell" : ""} ${
                isChatRoomPage
      ...
      {!isBattleRoute ? <LiveFloatingPlayer /> : null}
